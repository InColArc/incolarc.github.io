...
- [x] **Next iteration — free-text field under each section**
- [x] **Next iteration — export result as a PDF and send it on.** This is the designed "next step" so the person is no longer left alone with the insight: a forwardable artefact to build alliances with (the thing the eight-thousand-emails manager could finally send upward). *Keep generation client-side so "Keine Daten werden gesendet" stays true — user generates the PDF and forwards it themselves; no server round-trip.*
- [ ] English version — for international readers arriving via the printed Touchpoint link

**Fundament-Diagnose**
- [ ] German version built (`fundament.html`), wired onto the static house
